#include<temperature_layer/temp_layer.h>
...
PLUGINLIB_EXPORT_CLASS(temperature_layer_namespace::TempLayer, costmap_2d::Layer)
...
namespace temperature_layer_namespace
{
...
void TempLayer::onInitialize()
{
  ...
  dsrv_ = new dynamic_reconfigure::Server<temperature_layer::TemperatureLayerConfig>(nh);
  dynamic_reconfigure::Server<temperature_layer::TemperatureLayerConfig>::CallbackType cb = boost::bind(
...
void TempLayer::reconfigureCB(temperature_layer::TemperatureLayerConfig &config, uint32_t level)
{
...
void TempLayer::updateObservations(std::list<std::pair<unsigned int, float> > &updates)
{
  ...
  for (std::list<sensor_msgs::Image>::iterator obs = temperature_msg_buffer_copy_.begin(); obs != temperature_msg_buffer_copy_.end(); obs++) {
    ...
    try
    {
      ...
      tf_ -> lookupTransform(global_frame_, obs -> header.frame_id, ros::Time(0), transformStamped);
    }
